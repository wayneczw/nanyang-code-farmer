...
def build_model(
    ...
    lang_input_shape,
    output_shape,
    dropout_rate=0.1, kernel_regularizer=0,
    ...
    lang_input = Input(lang_input_shape, name='lang_input')
    lang = Dense(16)(lang_input)
    # lang = Dropout(dropout_rate)(lang)

    inputs = [title_input, nouns_input, numbers_input, img_input, lang_input]

    x = concatenate([title, nouns, numbers, img, lang])
    x = Dense(1024)(x)
...
def batch_iter(
    X_title, X_nouns, X_numbers, X_img, X_lang,
    y,
    ...
    def _data_generator():
        while True:
            ...
            for i in range(num_batches):
                ...
                X_lang_batch = [x for x in X_lang[start_index:end_index]]
                ...
                yield ({'title_input': np.asarray(X_title_batch),
                        ...
                        'lang_input': np.asarray(X_lang_batch),
                        },
...
def train(
    model,
    X_title_train, X_nouns_train, X_numbers_train, X_img_train, X_lang_train,
    y_train,
    X_title_val=None, X_nouns_val=None, X_numbers_val=None, X_img_val=None, X_lang_val=None,
    y_val=None,
    ...
    train_steps, train_batches = batch_iter(
        X_title_train, X_nouns_train, X_numbers_train, X_img_train, X_lang_train, y_train,
        batch_size=batch_size)

    if y_val is not None:
        val_steps, val_batches = batch_iter(
            X_title_val, X_nouns_val, X_numbers_val, X_img_val, X_lang_val, y_val,
            batch_size=batch_size)
...
def test(
    model,
    X_title_test, X_nouns_test, X_numbers_test, X_img_test, X_lang_test,
    lb, mapping,
    batch_size=128, **kwargs):
...
def main():
    ...
    argparser.add_argument('--train_img_features', type=str, default="", required=True, help='Path to folder which contains the x_image folder')
    argparser.add_argument('--test_img_features', type=str, default="", required=True, help='Path to folder which contains the x_image folder')
    ...
    for y in categorical_targets:
        ...
        train_dict['X_lang_train'] = lang_vec.fit_transform(train_df['lang'][train_dict['X_' + y + '_train_index']])
        ...
        if validate:
            ...
            train_dict['X_lang_val'] = lang_vec.transform(val_df['lang'][train_dict['X_' + y + '_val_index']])
        ...
        model = build_model(
            ...
            lang_input_shape=train_dict['X_lang_train'].shape[1:],
        ...
        test_dict['X_lang_test'] = lang_vec.transform(test_df['lang'].values)

        test_df[y] = test(**test_dict)
    #end for

    test_df.to_csv('./data/fashion_test_proba_img.csv', index=False)
#end def
